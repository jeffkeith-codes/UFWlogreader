...
WORKDIR = '/home/jak/Documents/pyprogs/UFWlogreader'
...
def make_restofline(string_in):
    ...
    for s in string_in.split():

        if re.match('\w+=.+', s):  # best case - a key and value pair

            (k,v) = s.split('=')
            # print('type 1 entry: ', x)
            #print('key: {}, value: "{}"'.format(k,v))
            restofline_dict[k] = v
        elif re.match('\w+=', s): # don't store these since there's no value
            #print('type 2 entry: ', x)
            pass
        elif re.match('\w+', s): # concatenate these into a flags field
...
def count_attribute(what_to_find):
    """ build a list, counting the occurrences of an attribute
        in the data """
    results = []
    for key in big.keys():
        results += [big[key][what_to_find]] # enclosed to avoid splitting
    return Counter(results)

def print_counts(counter_to_print, column_heading):
    """ print the collection.Counter with a heading """
    print(f'{column_heading:>17}{str("Count"):>10}')
    for key, value in counter_to_print.items():
...
def main():
    ...
    # show summary stats for a few attributes in the log:
    print_counts(count_attribute('SRC'), 'Src IP')
    print_counts(count_attribute('DST'), 'Dst IP')
    print_counts(count_attribute('MAC'), 'MAC Addr')
    print_counts(count_attribute('PROTO'), 'Protocol')
